...
class GetallSpider(scrapy.Spider):
    name = 'getall'
    # ミドルウエアを登録する
    costom_settings = {"DOWNLOADER_MIDDLEWARES": {"sakusibbs.selenium_middleware.SeleniumMiddleware": 0}
    }
    # リクエスト実行前にログインする
    def start_requests(self):
        # ログインページを開く
        url = 'https://uta.pw/sakusiibbs/users.php?action=login'
        selenium_get(url)
